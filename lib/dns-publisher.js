...
function getDnsProvider() {
  return (process.env.AMA_DNS_PROVIDER || "cloudflare").toLowerCase();
}

function getDnsTtlSeconds() {
  const raw = process.env.AMA_DNS_TTL_SECONDS;
  if (!raw) {
    return 60;
  }
  ...
  if (!Number.isFinite(ttl) || ttl <= 0) {
    throw new Error("AMA_DNS_TTL_SECONDS must be a positive number");
  }
...
export async function publishPaymentInstructionTxt({ recordName, txtValue }) {
  ...
  throw new Error(`Unsupported AMA_DNS_PROVIDER: ${provider}`);
}
